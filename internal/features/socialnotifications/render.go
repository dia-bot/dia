...
// renderComposed renders a subscription's composed MessageSpec into the
// announcement send: templated content and embeds plus composed button rows.
// Mirrors tickets' renderSpec/renderSpecRows so the shared dashboard editor's
// output posts identically across features.
func renderComposed(ctx context.Context, tmpl *templating.Engine, spec MessageSpec, data map[string]any, subID int64, kind string, fallbackColor int) (string, []*discordgo.MessageEmbed, []discordgo.MessageComponent) {
	...
	rows := renderComposedRows(spec, r, subID, kind)
	return content, embeds, rows
}

// renderLine renders one templated string against the announcement scope,
// returning the raw string on any template error so a typo never blanks an
// announcement.
...
// templated URL; any other button gets a social:act:<subID>:<kind>:<suffix>
...
// an announcement and are skipped.
func renderComposedRows(spec MessageSpec, r func(string) string, subID int64, kind string) []discordgo.MessageComponent {
	...
	for _, row := range spec.Components {
		...
		for _, c := range row.Components {
			...
			btn := discordgo.Button{
				...
				CustomID: actionCustomID(subID, kind, c.CustomIDSuffix),
				Disabled: c.Disabled,
			}
			if em := componentEmoji(c.Emoji); em != nil {
				btn.Emoji = em
