...
```
Discord  ──ws──▶  Nostrum shards  ──▶  Dia.Gateway.Consumer
                                            │  map_*  (Mapper)
                                            ▼
                                     Envelope JSON
                                            │
                              Dia.Gateway.Publisher (pool)
                                            ▼
...
Example — `SHARD_TOTAL=7`, `NODE_COUNT=3`:
...
The image is a multi-stage build (hexpm/elixir build stage → `debian:bookworm-slim`
