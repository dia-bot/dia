...
<div bind:this={rootEl} class="space-y-2" onfocusin={onFocusIn}>
	...
	<!-- Winner DM — a second surface, edited the same way, shown when DM is on. -->
				...
				<div class="rounded-lg bg-[#313338] px-4 py-3">
					<div class="flex gap-3.5">
						...
						<div class="min-w-0 flex-1">
							...
							<EmojiText
								class="w-full text-[13px] leading-[1.45] text-[#dbdee1]"
								placeholder={'🎉 You won {{ .Prize }}! Contact the host to claim.'}
								value={announce.dm_message ?? ''}
								onChange={(v) => (announce.dm_message = v)}
							/>
						</div>
					</div>
				</div>
