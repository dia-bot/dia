<script lang="ts">
	// The INLINE colour controls of the Figma-style picker — a saturation/value
	// square, eyedropper + hue + alpha sliders, a Hex field with opacity %, and
	// swatches. Extracted from ColorPicker so other pickers (PaintPicker's solid
	// tab and gradient stop editing) can embed the same controls without nesting
	// popovers. `value` is #RRGGBB, or #RRGGBBAA when opacity < 100.
	...
	// solid = the current colour WITHOUT alpha (for previews / gradients).
	...
	// a tiny checkerboard so transparency reads on the alpha track / swatches.
	const checker =
