...
<Select.Root type="single" bind:value items={options}>
	<Select.Trigger
		class="group flex w-full items-center justify-between gap-2 rounded-lg border border-line-strong bg-ink-2 outline-none transition-colors hover:border-faint focus:border-accent data-[state=open]:border-accent {dense
				? 'h-7 px-2.5 text-xs'
				: 'h-9 px-3 text-sm'}"
	>
