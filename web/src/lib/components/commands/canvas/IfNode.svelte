<script lang="ts">
	...
	const cond = $derived(((step?.spec ?? {}) as any).cond?.src ?? '');
...
<div
	class="step-node group/node relative w-[230px] rounded-xl border text-foreground transition-all duration-200
		{locked
...
>
	...
	<Handle
		...
		id="in-left"
		style="top: 18px; --dia-dot-dx: 119px"
	...
	>
		...
		<span class="min-w-0 flex-1 truncate text-[12.5px] font-semibold leading-tight text-foreground">If</span>
		{#if locked}
	...
	<!-- Footer: where the two paths leave the card -->
	<div
	...
	<!-- THEN bottom-left, ELSE bottom-right. -->
	<Handle
		type="source"
	...
	<Handle
		...
		style="left: 75%"
		isConnectable={!locked}
		class="!size-2.5 !border-2 !border-card !bg-muted-foreground/55 hover:!bg-foreground"
	/>
	{#if data.hasAfter}
		<!-- Older flows only: steps chained after the if (runs once either path
		     finishes). Leaves from the bottom centre — click the line to tuck
		     the chain into then/else instead. -->
		<Handle
			type="source"
			position={Position.Bottom}
			id="out"
			...
			class="!size-2 !border-2 !border-card !bg-muted-foreground/40 {locked && !outUnlocked
