<div align="center">
  ...
  **A modern, open-source Discord bot with a beautifully simple realtime dashboard.**
  ...
  Welcome images · Leveling & rank cards · Reaction & auto roles · Moderation & automod · Custom commands

  Slash-command native · Easily self-hostable · Shards across machines
</div>
...
```
                       Discord Gateway (WebSocket, sharded)
                                   │
                    ┌──────────────┴──────────────┐
                    │   gateway/  (Elixir/Nostrum) │   ← holds shard connections,
                    │   thin event pump            │     forwards events as JSON
                    └──────────────┬──────────────┘
                                   │  publish
                          ┌────────▼────────┐
                          │  NATS JetStream │   discord.events.<type>.<guild_id>
                          └────────┬────────┘
                    ┌──────────────┴───────────────┐
            consume │                               │ consume (realtime)
        ┌───────────▼──────────┐         ┌──────────▼───────────┐
        │ cmd/worker (Go)      │         │ cmd/api (Go, gin)    │
        │ plugins, interactions│         │ OAuth2, config CRUD, │
        │ XP, automod, imaging │         │ image previews, WS   │
        └───────────┬──────────┘         └──────────┬───────────┘
                    │                               │  WebSocket
            ┌───────▼───────┐               ┌───────▼────────┐
            │ Postgres+Redis│               │  web/ SvelteKit│  ← landing + dashboard
            └───────────────┘               └────────────────┘
```
...
```
gateway/         Elixir gateway (Nostrum → NATS)
